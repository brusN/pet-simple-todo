...
public class FolderController {
    ...
    @GetMapping("/all/{id}")
    public ResponseEntity<Object> getAllFolderTasks(@AuthenticationPrincipal UserEntity user, @PathVariable(value = "id") Long folderId) {
        var response = folderService.getAllFolderTasks(user, folderId);
        return ResponseEntity.ok(response);
    }
